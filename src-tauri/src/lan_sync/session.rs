...
fn apply_received(manager: &LanSessionManager, store: &Store, clip_type: &str, payload: &[u8]) {
    // 图片 = data url（utf-8）；文本 = 原文 utf-8
    let text = String::from_utf8_lossy(payload).to_string();
    if let Ok(Some(item)) = captured_item_from_payload(clip_type, &text) {
        ...
        manager.emit_clip_received(clip_type.to_string());
    }
}
    ...
    loop {
        tokio::select! {
            ...
            control = control_rx.recv() => match control {
                Some(ControlMsg::SendClip { clip_type, payload }) => {
                    let empty = payload.is_empty();
                    let msg = LanMessage::ClipPush { clip_type: clip_type.clone(), empty };
            ...
            read_result = conn.read_message() => match read_result {
                Ok((LanMessage::ClipPush { clip_type, empty }, payload)) => {
                    if !empty {
                        if let Some(data) = payload {
                            apply_received(&manager, &store, &clip_type, &data);
                        }
                    }
                }
                Ok((LanMessage::ClipRequest, _)) => {
                    match read_current_payload() {
                        Ok(Some((ct, data))) => {
                            let empty = false;
                            let msg = LanMessage::ClipResponse { clip_type: ct, empty };
                        ...
                        Ok(None) => {
                            let msg = LanMessage::ClipResponse { clip_type: "text".into(), empty: true };
                ...
                Ok((LanMessage::ClipResponse { clip_type, empty }, payload)) => {
                    if !empty {
                        if let Some(data) = payload {
                            apply_received(&manager, &store, &clip_type, &data);
                        }
...
mod tests {
    ...
    async fn connection_roundtrips_clip_push_with_payload() {
        ...
        client
            .write_message(
                &LanMessage::ClipPush { clip_type: "text".into(), empty: false },
                Some(b"hello"),
            )
            .await
            .unwrap();
    ...
    async fn empty_clip_push_has_no_payload() {
        ...
        let mut client = Connection::new(TcpStream::connect(addr).await.unwrap());
        client
            .write_message(&LanMessage::ClipPush { clip_type: "text".into(), empty: true }, None)
            .await
            .unwrap();
        server.await.unwrap();
    }
