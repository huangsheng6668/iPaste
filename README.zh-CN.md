...
- 多 OCR 引擎：支持本地引擎（macOS Vision、Windows PaddleOCR 快速/精确模式，以及日漫专用的 Manga-OCR 本地 ONNX sidecar）与 OpenAI 兼容云端端点（GPT-4o、GLM-4V、Qwen-VL、本地 Ollama/vLLM，支持自定义 System / User 提示词列表及语言占位符动态插值），可在结果窗与查看器中无缝直切。
...
两台 iPaste 通过一次性邀请票据配对：一台生成邀请，另一台凭票据加入，双方确认后互信。设备间经 QUIC 跨互联网直连（NAT 打洞失败时由中继转发密文），片段与整组分类在设备间端到端加密直传，接收端不存在的分类会自动创建。已配对设备可随时管理或撤销，断线后会自动退避重连。支持捕获即自动同步：文本类自动推送、图片与文件手动发送，可按设备调整或全局关闭。
